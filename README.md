...
本仓库同时服务于模板开发和用户写作参考，几个主要目录的用途如下：

- `source/`：模板的 `.dtx` 源代码、用户手册源码和相关资源。这里的文件仅供开发者维护模板使用。如需提取最新版格式文件，可以用 `l3build unpack` 命令，或下载 GitHub Actions 提供的 Artifacts。
- `template/`：作为标准写作示例的空白模板。
- `examples/`：用于收纳典型用法参考，例如定制附加页面、处理声明页等非通用但可复用的示例。
- `test/`：回归测试和编译测试文件，用于保证模板实现的稳定性。
- `scripts/` 和 `.github/`：自动化脚本与持续集成、发布流程配置。
